...
if globalCurrQuote == "0":
    print "<div id=\"invalidStockEntry\">"
    header = "invalid stock!"
    err = "The stock symbol you entered is not valid.  Please return to the portfolio page and try again"
    alertCard = errorCard(err, header)
    print alertCard
    print "</div>"
    print "</div>" # end buy-stocks div
    print "</body>"
    print "</html>"
    if con:
        con.close()
...
maxAMT = int(float(curr_balance) // float(globalCurrQuote))
stockSelectMessage = "With your current balance of $" + curr_balance + ", you can only afford to purchase:<br><br><h3><mark>" + str(maxAMT) +  "</mark></h3><br> shares of this stock."
stockSelectCard = generateCardWithBody("card text-white bg-info", "Stock to Purchase:", symbol, "Price per share: $" +globalCurrQuote + "<br><br><br>" + stockSelectMessage)
...
purchaseButton = "<button onmousedown=\"submitStockPurchase()\"type=\"button\" class=\"btn btn-success\">&nbsp;Submit Purchase!&nbsp;</button>"
purchaseCard = generateCardWithBody("card text-white bg-dark", "Submit purchase order:", "Submit Order<br><br><br>", purchaseButton)
print purchaseCard
print "</div>"
...
backButtonSelect = "<button onmousedown=\"backToStockSelectPage()\" type=\"button\" class=\"btn btn-danger\">&nbsp;Back to Select Stock Page Page&nbsp;</button>"
