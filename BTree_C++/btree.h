...
enum BTreeOp {BTREE_OP_INSERT, BTREE_OP_DELETE, BTREE_OP_UPDATE,BTREE_OP_LOOKUP, BTREE_OP_FINDLEAF, BTREE_OP_FINDPARENT};
...
class BTreeIndex {
 ...
 protected:
  ...
  // user added
  void InitNode(const KEY_T &key, const VALUE_T &value);
  void PromoteKey(SIZE_T *par, KEY_T *p_key, SIZE_T *p_ptr);
  void PrintNodeInterior(SIZE_T *node, int fromSanity=0);
  void PrintNodeLeaf(SIZE_T *node, int fromSanity=0);
  void splitNodeLeaf(SIZE_T *left_loc, SIZE_T *right_loc);
  void splitNodeInterior(SIZE_T *left_loc, SIZE_T *right_loc);
  void insertIntoLeaf(const KEY_T &key, const VALUE_T &value, SIZE_T *loc);
  void insertIntoInterior(SIZE_T *loc, KEY_T *new_key, SIZE_T *new_ptr, int fromInteriorSplit=0);
  void createTwoInteriorNodes(SIZE_T *left_loc, SIZE_T *right_loc);
  void copyRootOrInteriorNode(SIZE_T *to_fill_loc, SIZE_T *to_copy_loc);
  void findParent(SIZE_T *res_loc, SIZE_T *lookup_loc);
  void SanityCheckInternal(SIZE_T *curr_loc);
