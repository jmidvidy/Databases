...
// needs to create the "what" dynamically
constructWhat = function(){
    out = [];
    for (var key in input_cache){
	if (input_cache[key] == 1){
	    out.push(key);
	}
    }
    o = out.join(",");
...
// in input cache depending on boxes checked/unchecked
updateInputCache = function(param){
    if (input_cache[param] == 0){
	input_cache[param] = 1;
    }
    else {
	input_cache[param] = 0;
    ...
    whatStatement = constructWhat();
},

// update the cycle list passed to the back end
getCycleList = function(){
    var cyList = "";
    for (var key in cycle_cache){
	if (cycle_cache[key] == 1){
	    var a = key.substring(2,4);
	    var b = key.substring(7);
	    var update = a + b;
	    var curr = "\'" + update + "\',";
	    cyList = cyList + curr;
	}
    ...
    cyList = cyList.substring(0, cyList.length - 1);
...
// update cycles list based on 
// checked or unchecked boxes
clicked = function(item){
    var id = $(item).attr("id");
    if (!(id in cycle_cache)){
	cycle_cache[id] = 1;
    }
    else{
	if (cycle_cache[id] == 1){
    ...
    //console.log(cycle_cache);
    cycleList = getCycleList();
},


// gather form submit for INVITE user
sendInviteEmail = function(){
    console.log("got here");
    },

// do nothing callback function for
...
//
	...
	// update each section based off user input
	// global dictionary input_cache is altered
	// when useres check/uncheck an select box

	if (input_cache["committees"] == 1){
	    UpdateMapById("committee_data","COMMITTEE");
	}
	if (input_cache["candidates"] == 1){
	    UpdateMapById("candidate_data","CANDIDATE");
	}
	if (input_cache["individuals"] == 1){
	    UpdateMapById("individual_data", "INDIVIDUAL");
	}
	if (input_cache["opinions"] == 1){
		UpdateMapById("opinion_data","OPINION");
	    }
	UpdateClipRect();

	//don't forget to update summary statistics
...
getVisChart = function(id,title, n1, n2, v1, v2){
    try{
	google.charts.load('current', {'packages':['corechart']});
    }
    catch(err){
    }
    //console.log(title, n1, n2, v1, v2);

    google.charts.setOnLoadCallback(drawChart(id,title,n1,n2,v1,v2));
    
    //console.log(title, n1, n2, v1, v2);
    
    function drawChart(id,title, n1, n2, v1, v2) {

	console.log(id,title, n1, n2, v1, v2);

	var arr = [['Task','Hours Per Day'],[n1,v1],[n2,v2]];

	var data = google.visualization.arrayToDataTable(arr);


	var options = {'title':title, 'width':380, 'height':200};
...
// PART THREE :: update the summary statistics
// of each class
updateSummaryStatistics = function(){
    // update committeess
    if (input_cache["committees"] == 1){
       	//stored in form "REP_TOTAL, DEM_TOTAL"
	var stats = document.getElementById("CommitteeSummaryHidden").innerHTML;
	console.log(stats);
	var split = stats.split(",");
	//convert to Numberr
	var rep_total = Number(split[0]);
	var dem_total = Number(split[1]);
	var total = rep_total + dem_total;

	var rep_percent = Math.floor((rep_total / total) * 100);
	var dem_percent = Math.floor((dem_total / total) * 100);

	var rep_string = "From Reps: (" + rep_total + "/" + total + ") " + rep_percent + "%";
	var dem_string = "From Dems: (" + dem_total + "/" + total + ") " + dem_percent + "%";

	var HTML = "<br>" + rep_string + "<br>" + dem_string;
	console.log(HTML);
	getVisChart('piechartCommittee' ,'Committees Summary', 'D', 'R', dem_percent,  rep_percent );
	$("#CommitteeStatistics .inner-stats").html(HTML);	
    }
    else {
	$("#CommitteeStatistics .inner-stats").html("<br>Not selected.");
	$("#piechartCommittee").hide();
    }
    
    // update individuals
    if (input_cache["individuals"] == 1){
	//comes in form REP/DEM
	var stats = document.getElementById("IndividualSummaryHidden").innerHTML;
	console.log(stats);
	var split = stats.split(",");
	//convert to Number
        var rep_total = Number(split[0]);
        ...
        var HTML = "<br>" + rep_string + "<br>" + dem_string;
        console.log(HTML);
	getVisChart('piechartIndividuals', 'Indivduals Summary', 'D', 'R', dem_percent, rep_percent);
        $("#IndividualStatistics .inner-stats").html(HTML);
    }
    else {
	$("#IndividualStatistics .inner-stats").html("<br>Not selected.");
	$("#piechartIndividuals").hide();
    }
    
    // update opinions
    if (input_cache["opinions"] == 1){
	var stats = document.getElementById("OpinionSummaryHidden").innerHTML;
	console.log(stats);
	var split = stats.split(",");
	var std = split[0];
	var avg = split[1];
	...
	var std_string = "Standard Deviation: " + std_t.substring(0,5);
	var avg_string = "Average: " + avg_t.substring(0,5);
	
	var HTML = "<br>" + std_string + "<br>" + avg_string;
	console.log(HTML);
	$("#OpinionStatistics .inner-stats").html(HTML);
	...
	// DYNAMICALLY CHANGE BACKGROUND COLOR FOR DIFFERENT SHADES
	// USING CSS HEX CODES AND JQUERY .CSS BACKGROUND-CHANGE
	// POLICY

	//avg=.75;
	//isNeg = "False";
	//console.log(avg, isNeg);
	if (avg == 0){
	    newColor = "white";
	}
	...
	$("#colorBoxOpinions").show();
	$("#colorBoxOpinions").css({"background-color":newColor});

    }
    else {
	$("#OpinionStatistics .inner-stats").html("<br>Not selected");
	$("#colorBoxOpinions").hide();
    }
...
// the browser will call us back at the function NewData (given above)
       ...
       // uses global whatStatement composed when 
       // boxes are checked and/or unchecked
       ...
       //make sure to update summary statistics
       //try{
...
// We will put a google map into that division
	...
	// CHANGE LAT/LONG IF THE ELEMENT EXISTS ON THIS PAGE
	if ($('#give-opinion-data').length){
	    var newHREF = "rwb.pl?act=give-opinion-data&lat=" + lat + "&long=" + long;  
	    $("#give-opinion-data").attr("href",newHREF);
	}
